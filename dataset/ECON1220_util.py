...
def extract_text(page: pdf.Page) -> list[str]:
    ...
    # Remove the unnecessary title in lecture note
    pattern = r"\nECON1220 L1 – Ten Principles of Economics\n(\d+)\n"
...
def make_course_dict(notes_path: str, course_json_target_path = "/shared/project/CourseQA-RAG/dataset") -> Dict[]:
    ...
    course_value = {}
    for doc_path in os.listdir(notes_path):
    ...
    with open(course_json_target_path, "w", encode = "utf-8") as f:
        json.dump(course)
